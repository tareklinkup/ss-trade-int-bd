...
//			print_r($this->session->userdata("cheque"));
	...
	public function check_paid_submission($cheque_id = Null){
		...
		$cheque_info = $this->Check_model->check_data_by_id($cheque_id);
		...
		$data = array(
			...
			"CPayment_invoice"       =>$purchInvoice,
			"CPayment_customerID"    =>$cheque_info->cus_id,
			"CPayment_TransactionType"=>'CR',
			"CPayment_amount"        =>$cheque_info->check_amount,
			"CPayment_notes"         =>'Cheque payment',
			"CPayment_Paymentby"     =>'By Cheque',
			"CPayment_Addby"         =>$this->session->userdata("FullName"),
			"CPayment_brunchid"      =>$this->session->userdata("BRANCHid"),
			"CPayment_AddDAte"       =>date('Y-m-d'),
			"CPayment_status"        =>'a',
		);
		 $result = $this->mt->insert_payment("tbl_customer_payment", $data);
		 if($result){
		 	echo 1;
		 }else{
		 	echo 0;
