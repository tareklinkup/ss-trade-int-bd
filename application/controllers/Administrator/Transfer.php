    ...
    class Transfer extends CI_Controller{
        ...
        public function getTransferDetails() {
            ...
            $transferDetails = $this->db->query("
                ...
                left join tbl_transfermaster tm on tm.transfer_id = td.transfer_id
                where td.transfer_id = ?
            ", $data->transferId)->result();

            $transferDetails = array_map(function($transferDetails){
                $transferDetails->serial = $this->db->query("SELECT * FROM tbl_product_serial_numbers WHERE ps_prod_id = ? and ps_transfer_to =?",[$transferDetails->product_id, $transferDetails->transfer_to] )->result();
        ...
        public function deleteTransfer() {
            ...
            try{
                ...
                foreach($oldTransferDetails as $oldDetails) {
                    ...
                    // remove transfer product serial
                    $oldTransferProduct = $this->db->query("select ps_prod_id, ps_transfer_from from tbl_product_serial_numbers where ps_prod_id = ? and ps_transfer_from is not null", $oldDetails->product_id)->result();
                    foreach($oldTransferProduct as $value) {
                        $this->db->query("
                            update tbl_product_serial_numbers 
                            set ps_brunch_id = ?
                            where ps_prod_id = ?
                        ", [$value->ps_transfer_from, $value->ps_prod_id]);
                    }
