						...
						<div class="col-sm-6">
							<form v-on:submit.prevent="addToCart">
								...
								<div class="form-group">
									...
									<div class="col-sm-9"> 
										<v-select v-bind:options="saleSerial" v-model="serialIn" label="ps_serial_number"></v-select>
									</div>
								</div>
								...
								<div class="form-group">
									...
									<div class="col-sm-9"> 
										<v-select v-bind:options="availableStock" v-model="serialOut" label="ps_serial_number"></v-select>
									</div>
								</div>
	...
	new Vue({
		...
		watch: {
            selectedCustomer(customer) {
                ...
                this.selectedCustomer.Customer_Mobile = customer.Customer_Mobile;
                this.selectedCustomer.Customer_Address = customer.Customer_Address;
                this.replace.customerId = customer.Customer_SlNo;
            },
			async selectedProduct(product) {
				if(product == undefined) return;
				await this.getProductSerial(product.Product_SlNo);
				await this.getAvailableProductStock(product.Product_SlNo);
			}
		},
		created(){
            this.getCustomers();
		...
		methods:{
			...
			async getProductSerial(productId) {
				await axios.post('/get_sales_serial', { productId: productId})
				.then(res => {
					this.saleSerial = res.data
				})
			},
			async getAvailableProductStock(productId) {
				await axios.post('/get_Serial_By_Prod', { prod_id: productId })
				.then(res => {
					this.availableStock = res.data;
				})
			},
			addToCart(){
				...
				let product = {
					...
					serialIn: this.serialIn.ps_serial_number,
					serialOut: this.serialOut.ps_serial_number
				}

				let checkSerialIn = this.cart.findIndex(item => item.productId == this.selectedProduct.Product_SlNo &&  item.serialIn == this.serialIn.ps_serial_number)
			...
			async saveReplace(){
				...
				await axios.post('/add_replace', data)
				.then(res => {
					if(res.data.success) {
