	...
	<div class="row" style="border-bottom: 1px solid #ccc;padding: 3px 0;">
		<div class="col-md-12">
			<form class="form-inline" id="searchForm" @submit.prevent="getSearchResult">
				...
				<div class="form-group" v-bind:style="{display: searchType == '' || searchType == 'user' || searchType == 'customer' || searchType == 'employee' || searchType == 'service' ? '' : 'none'}">
	...
	<div class="row" style="margin-top:15px;display:none;" v-bind:style="{display: sales.length > 0 ? '' : 'none'}">
		...
		<div class="col-md-12">
			<div class="table-responsive" id="reportContent">
				<table 
					class="record-table" 
					v-if="(searchType == '' || searchType == 'user' || searchType == 'customer' || searchType == 'employee' || searchType == 'service') && recordType == 'with_details'" 
					style="display:none" 
					v-bind:style="{display: (searchType == '' || searchType == 'user' || searchType == 'customer' || searchType == 'employee' || searchType == 'service') && recordType == 'with_details' ? '' : 'none'}"
					>
					<thead>
						<tr>
							<th>Invoice No.</th>
				...
				<table 
					class="record-table" 
					v-if="(searchType == '' || searchType == 'user' || searchType == 'customer' || searchType == 'employee' || searchType == 'service') && recordType == 'without_details'" 
					style="display:none" 
					v-bind:style="{display: (searchType == '' || searchType == 'user' || searchType == 'customer' || searchType == 'employee' || searchType == 'service') && recordType == 'without_details' ? '' : 'none'}"
					>
					<thead>
						<tr>
							<th>Invoice No.</th>
				...
				<table 
					...
					<tbody>
						<tr v-for="sale in sales"> 
							...
							<td style="text-align:right;">{{ sale.SaleDetails_Rate*sale.SaleDetails_TotalQuantity }}</td>
						</tr>
					</tbody>
					<tfoot>
						<tr style="font-weight:bold;">
							...
							<td style="text-align:right;">{{ sales.reduce((prev, curr) => { return prev + parseFloat(curr.SaleDetails_Rate*curr.SaleDetails_TotalQuantity)}, 0) }}</td>
						</tr>
					</tfoot>
				</table>
			</div>
	...
	new Vue({
		...
		methods: {
			...
			getSearchResult(){
				...
				if(this.searchType == '' || this.searchType == 'user' || this.searchType == 'customer' || this.searchType == 'employee' || this.searchType == 'service'){
					this.getSalesRecord();
				} else {
					this.getSaleDetails();
				}
