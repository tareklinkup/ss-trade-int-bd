...
class Customer extends CI_Controller
{
    ...
    public function getCustomerDue(){
        ...
        if(isset($data->customerId) && $data->customerId != null){
            $customerIdClause = " and c.Customer_SlNo = '$data->customerId'";
        }
        ...
        $dueResult = $this->db->query("
            ...
            from tbl_customer c
            where c.Customer_brunchid = '$branchId' $customerIdClause
        ")->result();
    ...
    public function addCustomerPayment(){
        ...
        try{
            ...
            $message = "Dear {$sendToName},\nThanks for your payment. Received amount is tk. {$paymentObj->CPayment_amount}. Current due is tk. {$currentDue}";
    ...
    public function addCustomer()
    {
        ...
        try{
            ...
            $customerMobileCount = $this->db->query("select * from tbl_customer where Customer_Mobile = ?", $customerObj->Customer_Mobile)->num_rows();
    ...
    public function updateCustomer()
    {
        ...
        try{
            $customerObj = json_decode($this->input->post('data'));
            $customerMobileCount = $this->db->query("select * from tbl_customer where Customer_Mobile = ? and Customer_SlNo != ?", [$customerObj->Customer_Mobile, $customerObj->Customer_SlNo])->num_rows();
            if($customerMobileCount > 0){
                $res = ['success'=>false, 'message'=>'Mobile number already exists'];
                echo Json_encode($res);
                exit;
