const purchaseInvoice = Vue.component('purchase-invoice', {
                            ...
                            <tbody>
                                <tr v-for="(product, sl) in cart">
                                    ...
                                    <td>{{ product.Product_Name }} <br> (
                                        <span v-for="(SerialCart, ind) in serialStore.filter(prod_sl => prod_sl.ps_prod_id == product.Product_IDNo)">
                                            {{SerialCart.ps_serial_number}}<span v-if="serialStore.length > 1">, </span>
                                        </span>
                                    )
                                    </td>
                                ...
                                <td></td>
                                <td>Total : </td>
                                <td>{{ cart.reduce((pre,next)=>{  return pre+ parseFloat(next.PurchaseDetails_TotalQuantity) },0) }}</td>
                                <td></td>
                                <td></td>
                                <td></td>
                            </tr>
    ...
    methods:{
        async getPurchase(){
            await axios.post('/get_purchases', {purchaseId: this.purchase_id}).then(res=>{
                this.purchase = res.data.purchases[0];
                this.cart = res.data.purchaseDetails;
            })
        },
        async getPurchaseImei(){
            await axios.post('/get_purchases_serial',{purchaseId:this.purchase_id}).then(res=>{
