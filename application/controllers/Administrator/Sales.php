...
class Sales extends CI_Controller {
    ...
    public function get_serial_report(){
        ...
        $res['purchaseReport'] =  $this->db->query("
            select s.*, pm.PurchaseMaster_InvoiceNo as invoiceNo,
            pm.PurchaseMaster_OrderDate as purchaseDate,
            sn.ps_serial_number, pd.PurchaseDetails_Rate as purchaseRate,
            ...
            left join tbl_brunch b on b.brunch_id = pm.PurchaseMaster_BranchID
            left join tbl_product p on p.Product_SlNo = sn.ps_prod_id
            where sn.ps_serial_number = ? 
        ", [$data->serial])->result();
     ...
     public function serial_return(){
        ...
        $this->db->where('ps_serial_number', $data->serial_number);
        $result = $this->db->update('tbl_product_serial_numbers', $datas);
        if ($result) {
            echo 'return';
        }
    }
    public function send(){
        $data = $this->db->query("SELECT * FROM tbl_purchasedetails")->result();
        foreach ($data as $key => $value) {
            $rate = $value->PurchaseDetails_Rate;
            $dis_per = $value->PurchaseDetails_Discount/100;
            $total_dis = $rate-($rate * $dis_per);
            $data = array( 
                'purchase_rate'      => $value->PurchaseDetails_Rate, 
                'purchase_discount'      => $value->PurchaseDetails_Discount,
                'purchase_total'=>$total_dis
            );
            $this->db->where('purchase_details_id', $value->PurchaseDetails_SlNo);
            $this->db->update('tbl_product_serial_numbers', $data);
        }
    ...
    function get_return_serial_number(){
        $serial_number = $this->input->post('serial_number');
        $get_ps_detail = $this->db->query("SELECT * FROM tbl_product_serial_numbers WHERE ps_serial_number='$serial_number'")->result_array();
        $ps_prod_id = $get_ps_detail[0]['ps_prod_id'];
        $sales_details_id = $get_ps_detail[0]['sales_details_id'];

        $get_m_detail =  $this->db->query("SELECT SaleMaster_IDNo FROM tbl_saledetails WHERE  SaleDetails_SlNo='$sales_details_id'")->result_array();
        
        $SaleMaster_IDNo = $get_m_detail[0]['SaleMaster_IDNo'];
        $serial_detail = $this->db->query("select ps.*,pd.SaleDetails_Rate,p.Product_Name,p.Product_SlNo
        FROM tbl_product_serial_numbers as ps 
        INNER JOIN tbl_saledetails as pd
        ON pd.Product_IDNo='$ps_prod_id' AND pd.SaleMaster_IDNo ='$SaleMaster_IDNo'
        INNER JOIN tbl_product as p 
        ON p.Product_SlNo ='$ps_prod_id'
        WHERE  ps.ps_serial_number ='$serial_number'")->result();
        ...
        foreach($serial_detail as $key=>$value){
            ...
            $html .='</tr>
                     <tr>
                     <td style="text-align: center;">'.$value->ps_serial_number.'</td>
                     <td style="text-align: center;">'.$value->Product_Name.'</td>
                     <td style="text-align: center;">'.$value->SaleDetails_Rate.'</td>
                     <td style="text-align: center;">'.$value->SaleDetails_Rate.'</td>
                     <td style="display: flex;align-content: center;justify-content: center;"><button style="width: 118px;padding: 7px;margin: 2px;border: navajowhite;" class="btn btn-success btn-sm" id="parchase_return_btn" data-id="'.$value->ps_serial_number.'" data-prod_id="'.$value->Product_SlNo.'" data-amount="'.$value->SaleDetails_Rate.'"> Return</button></td>
                 </tr>';
    ...
    function selectCustomer()  {
     ...
     public function sale_serial_return(){
        $data = json_decode($this->input->raw_input_stream);
        
        $datas = array(
            'ps_s_r_status'=>'yes',
            'ps_s_status'=>'no',
            'ps_s_r_amount'=> $data->return_amout
        );
        $this->db->where('ps_serial_number', $data->serial_number);
        $result = $this->db->update('tbl_product_serial_numbers', $datas);
    ...
    function GetSerialList(){
        ...
        $serial = $this->db->query("SELECT ps.*,ps.ps_s_discount as SaleDetails_Discount,ps.ps_prod_id,p.*,u.Unit_Name,pc.ProductCategory_Name
        FROM  tbl_product_serial_numbers as ps
        INNER JOIN tbl_product as p 
        ...
        ")->result();
         ...
         $serial = array_map(function($item){
                $item->quantity =  ['quantity' => 1,'SaleDetails_Discount'=>0];
                return $item;
            }, $serial);
        echo json_encode($serial);
    }
    function  GetSerialListService(){
        $branch_id = $this->session->userdata("BRANCHid");
        $serial = $this->db->query("SELECT ps.ps_serial_number,ps.purchase_date,ps.ps_prod_id,p.*,u.Unit_Name,pc.ProductCategory_Name
        FROM  tbl_product_serial_numbers as ps
        INNER JOIN tbl_product as p 
        ON ps.ps_prod_id = p.Product_SlNo
        INNER JOIN tbl_unit as u 
        ...
        INNER JOIN tbl_productcategory as pc
        ON p.ProductCategory_ID= pc.ProductCategory_SlNo
        WHERE ps.ps_s_status ='yes' 
        AND ps.ps_brunch_id='$branch_id'")->result();
        echo json_encode($serial);
    } 
    function GetServiceStatus(){
        $data = json_decode($this->input->raw_input_stream);
        $Serial = $this->db->query("SELECT * FROM tbl_services_details WHERE s_d_serial=?",$data->Serial)->num_rows();
        echo json_encode($Serial);
    }
    public function addSales(){
        ...
        try{
            ...
            $sales = array(
                ...
                'reference' => $data->sales->reference
            );
            ...
            foreach($data->cart as $cartProduct){
                ...
                $saleDetails = array(
                    ...
                    'SaleDetails_Discount'=> $cartProduct->SaleDetails_Discount,
                    'Discount_amount'=> $cartProduct->purchase_discount
                );
                ...
                foreach($cartProduct->SerialStore as $key => $value){
                    $data = array( 
                        'ps_s_status'=> 'yes',
                        'ps_s_r_status'=>'no',
                        'ps_s_r_amount'=>0,
                    ...
                    $this->db->where('ps_serial_number', $value->ps_serial_number);
                    $this->db->update('tbl_product_serial_numbers', $data);
                }
    ...
    public function getSaleDetails(){
        ...
        $saleDetails = $this->db->query("
            select 
                sd.*,
                sum(sd.SaleDetails_TotalQuantity) as SaleDetails_TotalQuantity,
                p.Product_Name,
            ...
            $clauses  group by sd.Product_IDNo
        ", $this->sbrunch)->result();
    ...
    public function getSalesRecord(){
        ...
        if(isset($data->isService) && $data->isService != ''){
            $clauses .= " and sm.is_service = '$data->isService'";
        }
        ...
        try{
            ...
            foreach($currentSaleDetails as $product){
                ...
                // Serial Update
                     ...
                     $dataArr = array(  
                         ...
                         'ps_s_r_amount'=>0,
                         'ps_s_status'=> 'no',
                         //'sales_details_id'=>$product->SaleDetails_SlNo
                     );
                     $this->db->where('sales_details_id', $product->SaleDetails_SlNo);
                     $this->db->update('tbl_product_serial_numbers', $dataArr);
            ...
            foreach($data->cart as $cartProduct){
                ...
                ", [$cartProduct->quantity, $cartProduct->productId, $this->session->userdata('BRANCHid')]);
                 ...
                 foreach ($cartProduct->SerialStore as $keyserial => $valueserial) {
                    // Insert Serial
                   // $check_available = $this->db->query("SELECT * FROM tbl_product_serial_numbers WHERE ps_serial_number=?",$valueserial->ps_serial_number)->num_rows();
                   ...
                   //   $this->db->insert('tbl_product_serial_numbers', $dataArr);    
                  // }else{
                    $dataArr = array(  
                         'ps_s_r_status'=>'no',
                         'ps_s_r_amount'=>0,
                     ...
                     // $this->db->where('sales_details_id', $product->SaleDetails_SlNo);
                     $this->db->where('ps_serial_number',$valueserial->ps_serial_number);
                     $this->db->update('tbl_product_serial_numbers', $dataArr);
    ...
    public function remove_cart_serial(){
        $data = json_decode($this->input->raw_input_stream);
        foreach($data as $key=>$value){
                     $data = array( 
                         'ps_s_status'=> 'no'
                     ...
                     $this->db->where('ps_serial_number', $value->ps_serial_number);
                     $this->db->where('sales_details_id', $value->sales_details_id);
                     $this->db->update('tbl_product_serial_numbers', $data);
        }
...
//    }
    ...
    public function branchwiseProductSale() {
        $access = $this->mt->userAccess();
        if(!$access){
            redirect(base_url());
        }
        
        $data['title'] = "Branchwise Product Sales";
        $data['content'] = $this->load->view('Administrator/sales/branchwise_sales', $data, true);
        $this->load->view('Administrator/index', $data);
    }
}