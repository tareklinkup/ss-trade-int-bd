...
class Purchase extends CI_Controller
{
    ...
    public function getPurchases(){
        ...
        if(isset($data->purchaseId) && $data->purchaseId != null){
            $res['purchaseDetails'] = array_map(function($purchase){
                $purchase->serial = $this->db->query("SELECT * FROM tbl_product_serial_numbers WHERE purchase_details_id =?  ",$purchase->PurchaseDetails_SlNo)->result();
                return $purchase;
            },  $res['purchaseDetails']);
        }
    ...
    public function getPurchaseDetailsForReturn(){
        ...
        $purchaseDetails = $this->db->query("
            ...
            AND pd.Product_IDNo = prd.PurchaseReturnDetailsProduct_SlNo
        ...
        array_map(function($details){
            $details->serials = $this->db->query("
                ...
                where purchase_details_id = $details->PurchaseDetails_SlNo
            ")->result();
        }, $purchaseDetails);
                        ...
                        foreach ($serials as $serial) {
                            if ($serial->is_return) {
                                $product_serial = array(
                                    'ps_p_r_status'=> 'yes',
                                    'ps_p_r_amount'=> $product->return_amount
                                );
    ...
    public function returnsIemi()
    {
        ...
        $data['title'] = "Purchase Return";
        $data['content'] = $this->load->view('Administrator/purchase/purchase_serial_return', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }
    
    public function purchase_serial_return(){
        $data = json_decode($this->input->raw_input_stream);
    ...
    function get_return_serial_number(){
        ...
        $serial_detail = $this->db->query("select ps.*,pd.PurchaseDetails_Rate,p.Product_Name,p.Product_SlNo
    ...
    function delete_serials(){ 
        ...
        foreach($data->product as $key=>$value){
            ...
            $this->db->query("delete from tbl_product_serial_numbers where ps_serial_number = ?", $value->serialNumber);
    ...
    public function addPurchase()
    {
        ...
        try{
            ...
            foreach($data->cartProducts as $product){
                ...
                foreach ($product->SerialCartStore as $serialkey => $serialvalue) {

               $rate = $serialvalue->purchaseRate;
               $dis_per = $serialvalue->discount/100;
               $total_dis = $rate-($rate * $dis_per);

               $data = array(
                'ps_prod_id'=>$serialvalue->productId,
                'ps_serial_number'=>$serialvalue->serialNumber,
                ...
                'purchase_rate'=>$serialvalue->purchaseRate,
                'purchase_discount'=>$serialvalue->discount,
    ...
    public function updatePurchase()
    {
        ...
        try{
            ...
            ps_purchase_inv=? and ps_serial_number=? and ps_prod_id=?',[$purchase_inv,$serialvalue->serialNumber,$serialvalue->productId])->num_rows();
            if($checkAvaImei < 1){
                $rate = $serialvalue->purchaseRate;
                $dis_per = $serialvalue->discount/100;
                $total_dis = $rate-($rate * $dis_per);
             ...
             $this->db->insert("tbl_product_serial_numbers",$data);
                ...
                $this->db->query('update tbl_product_serial_numbers SET  purchase_details_id=? WHERE ps_purchase_inv=? and ps_serial_number=? and ps_prod_id=? ',[$purchase_d_id,$purchase_inv,$serialvalue->serialNumber,$serialvalue->productId]);
            }
    ...
    public function getPurchasesImei(){
        ...
        $serials = $this->db->query("SELECT ps_serial_number,ps_prod_id FROM tbl_product_serial_numbers WHERE ps_purchase_inv=?",$purchaseInvoice)->result();
