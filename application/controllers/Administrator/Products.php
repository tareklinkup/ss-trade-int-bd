...
class Products extends CI_Controller {
    ...
    public function checkImeiNumber(){
        ...
        $query = $this->db->query("SELECT * FROM tbl_product_serial_numbers WHERE ps_serial_number=?",$obj->get_serial_number);
    ...
    public function SerialWiseCustomer(){
        $obj = json_decode($this->input->raw_input_stream);
        $data = $this->db->query("SELECT sales_details_id FROM tbl_product_serial_numbers WHERE ps_serial_number=?",$obj->Serial)->result_array();
        ...
        $stock = $this->db->query("
            select * from(
                select
                    ...
                    ifnull((select sum(purchase_total) as purchase_total from tbl_product_serial_numbers 
                        where ps_prod_id=p.Product_SlNo AND ps_p_r_status = 'no' 
                        AND (ps_s_status IS NULL OR ps_s_status = 'no' OR ps_s_r_status = 'yes') 
                        AND ps_brunch_id='".$BRANCHid."' 
                    ), (select stock_value)) as purchase_total_am,

                    (select sum(PurchaseDetails_TotalAmount) from tbl_purchasedetails 
                        where Product_IDNo=p.Product_SlNo
                    ) as stockValue
    ...
    public function getSaleSerials() {
        $data = json_decode($this->input->raw_input_stream);

        $clauses = "";
        if(isset($data->productId) && $data->productId != '') {
            $clauses .= " and ps_prod_id = $data->productId";
        }

        $serial = $this->db->query("
            select 
                ps.*
            from tbl_product_serial_numbers ps
            where ps.ps_brunch_id = ?
    ...
    public function getSaleSerialReplace() {
        $data = json_decode($this->input->raw_input_stream);
...
//		 echo $brand; die();
   ...
   function barcode($kode){
	...
	// show all branch product stock
	
	public function allBranchProductStock() {
        $access = $this->mt->userAccess();
        if(!$access){
            redirect(base_url());
        }
        $data['title'] = "Product Stock";
        $data['content'] = $this->load->view('Administrator/products/product_stock', $data, TRUE);
        $this->load->view('Administrator/index', $data);
    }

    public function getAllBranchProductStock() {
        $data = json_decode($this->input->raw_input_stream);
        $stock = $this->db->query("
            select
                ci.*,
            ...
            join tbl_product p on p.Product_SlNo = ci.product_id
            left join tbl_unit u on u.Unit_SlNo = p.Unit_ID
            left join tbl_brunch b on b.brunch_id = ci.branch_id
            where p.status = 'a'
            and p.is_service = 'false'
            and ci.product_id = ?
